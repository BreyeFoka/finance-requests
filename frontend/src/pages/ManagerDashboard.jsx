...
const ManagerDashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchApprovedRequests = async () => {
      try {
        const response = await axios.get('/api/manager/approved-requests');
  ...
  const handleAddEmployee = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('/api/manager/employees', newEmployee);
  ...
  const handleRemoveEmployee = async (id) => {
    try {
      const response = await axios.delete(`/api/manager/employees/${id}`);
      if (response.data.success) {
  ...
  const handleExport = async () => {
    try {
      const response = await axios.get('/api/manager/approved-requests/export', {
        responseType: 'blob'
      });
